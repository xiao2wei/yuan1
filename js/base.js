...
ajaxTip.prototype = {
    /**
     * 显示提示
     * @param String status 状态，正确：'ok', 错误：'err'
     * @param String tip 提示文字内容
     * @param Number time 提示消失倒计时，默认为0
     */
...
/**
 * 用户鉴权
 * Created by Arimis Wang on 2015/4/29.
 * @depandency  Aes Encrypt lib & Jquery.cookie
 */
//document.write("<script type='text/javascript' src='js/CryptoJSv3.1.2/components/core-min.js'></script>");
//document.write("<script type='text/javascript' src='js/CryptoJSv3.1.2/rollups/aes.js'></script>");
//var app_key = "cn.kela";
function checkLogin(showLoginDialog) {
    //var isAuth = CryptoJS.AES.decrypt($.cookie(CryptoJS.AES.encrypt('user_auth', app_key)), app_key);
    //var uid = CryptoJS.AES.decrypt($.cookie(CryptoJS.AES.decrypt('user_info', app_key)), app_key);

    var isAuth = $.cookie('user_auth');
    var user = $.parseJSON($.cookie('user_info'));
    if(isAuth == 1 && user) {
        return user;
    }
    //todo 弹窗提示登录
    ...
    $.cookie('login_referrer_url',window.location.href, {domain:window.JsDomain, path: "/"});
    return false;
}
...
//处理中文在base64编码时出错问题：javascript获取中文时，是按utf16编码的，所以要先转码为utf8
...
function isMobile(strMobile){
    ...
    return patternMobile.test(strMobile);
}
